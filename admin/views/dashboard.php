...
<div class="tab-container">
  ...
  wpsi_render_tab_content(
    'post-types',
    'Post Types',
    ['Type', 'Label', 'Location', 'Last Used (Date & Time)'],
    $post_type_rows
  );
...
if (file_exists($log_file)) {
    ...
    if ($total_lines > $last_sent_index) {
        ...
        if (!empty($new_logs_to_send)) {
            // Manually hardcoded emails (for testing only)
...
if (isset($_POST['wpsi_clear_logs']) && check_admin_referer('wpsi_clear_logs_action')) {
    ...
    update_option('wpsi_last_sent_log_index', 0); // Reset sent index
    $log_cleared = true;
}
    ...
    if (!$('#wpsi-ai-chatbox').length) {
        const chatHtml = `
            <div id="wpsi-ai-chatbox" style="display:none; position:fixed; bottom:20px; right:20px; width:700px;
                ...
                <div style="background:#252525; color:#fff; padding:12px 16px; font-weight:bold; position:relative;">
                    AI Code Assistant
                ...
                <div id="wpsi-chat-messages" style="flex:1; padding:15px; overflow-y:auto; display:flex; flex-direction:column; gap:10px; background:#1E1E1E;">
                    <!-- Messages -->
                </div>
                ...
                <div style="padding:10px; border-top:1px solid #ddd; display:flex;background-color:#252525;">
                    ...
                    <button id="wpsi-send-btn" style="margin-left:8px; padding:8px 12px; background:#4b6cb7; color:#fff; border:none; border-radius:6px; cursor:pointer;">
...
<script>
  ...
  document.getElementById("exportExcel").addEventListener("click", () => {
    ...
    document.querySelectorAll("table[data-title]").forEach(table => {
      ...
      // Temporarily show all paginated rows
      const hiddenRows = [];
      ...
      // Convert table to sheet and append
      const ws = XLSX.utils.table_to_sheet(table);
      XLSX.utils.book_append_sheet(wb, ws, title);

      // Restore hidden rows to maintain pagination
    ...
    XLSX.writeFile(wb, `${themeName}_Site_Inspector_report.xlsx`);
  });
